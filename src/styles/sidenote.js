document.addEventListener('DOMContentLoaded', () => {
    if (window.matchMedia('(min-width: 1000px)').matches) {
        const mainRect = document.querySelectorAll('main > article')[0].getBoundingClientRect();
        const notes = document.querySelectorAll('.footnotes > ol > li');
        const numbers = Array.from(document.querySelectorAll("a[href*='user-content-fn']"));
